...
Keep `RogueFarmGame` application-specific by moving reusable engine and procgen tooling out of the game class.

## Context

`RogueFarmGame` previously owned reusable terrain statistics, greater realm debug colourization, SDL surface generation, raw texture conversion, and retained UI rendering in addition to application-facing procgen controls and preview composition.

## Acceptance Criteria

- Audit `RogueFarmGame` and classify each responsibility as application-specific or reusable engine/tooling behavior.
- Move greater realm terrain counting and debug-map visualization out of `RogueFarmGame` into an appropriate non-game module.
...
- Moved the application-specific control panel into `GreaterRealmDebugPanel` under `src/game`.
- Kept generator settings, regeneration timing, preview entity placement, scale, and texture replacement orchestration in `RogueFarmGame`.
