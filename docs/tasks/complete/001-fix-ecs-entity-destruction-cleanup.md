...
- Keep tests in a separate target that is available only when `REALM_BUILD_TESTS=ON` and is excluded from normal runtime builds.
- Verify Debug and Release builds.

## Verification

- Debug runtime and all Debug test targets build successfully.
