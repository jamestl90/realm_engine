...
- **Climate**: `GreaterRealmClimateMap` is a versioned derived map with one climate cell per canonical terrain cell and a fingerprinted source-terrain identity. It provides fixed-scale `temperature_normal` and `precipitation_normal` climatological tendencies, not current weather or mutable terrain fields.
