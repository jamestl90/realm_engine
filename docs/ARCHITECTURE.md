...
### World Simulation Climate And Weather (future)
- **Seasonal evaluation**: A future world-simulation/application-facing layer evaluates seasonal temperature and precipitation tendencies from annual normals, explicit seasonal profiles, stable coordinates, and deterministic calendar inputs. Seasonal samples are derived/cacheable data with their own profile and algorithm versions; they are not saved terrain fields and do not affect biome identity.
