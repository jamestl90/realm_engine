...
- **Annual climate normals**: `GreaterRealmClimateMap` is a versioned derived map with one climate cell per canonical terrain cell and a fingerprinted source-terrain identity. It provides fixed-scale `temperature_normal` and `precipitation_normal` climatological tendencies, not current weather, seasonal samples, or mutable terrain fields.
- **Biome**: `GreaterRealmBiomeMap` is a versioned derived assignment map produced by deterministic precedence over validated application-supplied rules. Applications own opaque biome IDs, definitions, names, colours, art, resources, and gameplay policy; procgen owns only the generic evaluator and source identity checks.
- **Climate layer boundary**: Annual climate normals are stable biome-driving geography. Seasonal climate evaluation consumes those normals plus deterministic calendar inputs to produce predictable warm/cool and wet/dry offsets. Seasonal evaluation must not regenerate terrain, relabel biomes, or rewrite climate normals.
...
- **World simulation boundary**: Current rainfall, soil moisture, runoff, active river discharge, and other momentary environmental effects are runtime world state if gameplay later needs them. They are not generated climate normals, biome inputs, or terrain mutation sources.
- **MountainPeaks**: Deterministic spaced peak selection and jagged multi-source mountain distance fields
- **GreaterRealm inspection**: Build-configurable, engine-neutral terrain/climate/biome statistics and RGBA visualization are combined by the application with seasonal climate inspection. Shipped Debug and Release presets include the unified selector and explicit calendar control; biome colours remain application supplied with a neutral engine fallback.
...
### World Simulation Climate
- **Seasonal temperature evaluation**: `world::SeasonalTemperatureMap` is a versioned derived map that evaluates predictable temperature offsets from annual temperature normals, stable terrain coordinates, explicit seasonal settings, profile identity, and normalized year fraction. Northern and southern phase responses blend smoothly through an explicit equatorial transition band. It is not part of procgen climate identity and does not affect biome assignment.
- **Seasonal precipitation evaluation**: `world::SeasonalPrecipitationMap` is a versioned derived map that evaluates wet/dry seasonal multipliers from annual precipitation normals, explicit seasonal settings, profile identity, and normalized year fraction. Opposite hemisphere wet phases use the same smooth equatorial handoff, and climatological transport is never treated as runtime wind.
- **Persistence**: Annual climate and biome maps remain regenerated from versioned inputs by default. Seasonal samples can be recomputed from calendar/profile inputs. Runoff, soil moisture, and discharge remain future world/region simulation state.
- **Queries**: `world::ClimateWeatherSample` distinguishes stable annual climate, seasonal tendency, and stable biome ID so consumers do not accidentally treat seasonal values as biome-defining climate.
- **Inspection boundary**: `game::GreaterRealmInspectionView` and the application-side climate compositor visualize seasonal climate without making procgen depend on mutable world state. Inspection time changes rebuild seasonal preview state only and never regenerate or relabel biomes.
