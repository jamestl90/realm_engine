...
- **Climate layer boundary**: Annual climate normals are stable biome-driving geography. Future seasonal climate evaluation may consume those normals plus deterministic calendar inputs to produce predictable warm/cool or wet/dry offsets. Future runtime weather may consume normals, seasonal tendencies, and prior weather state to produce current conditions. Neither seasonal evaluation nor runtime weather may regenerate terrain, relabel biomes, or rewrite climate normals.
...
- **GreaterRealmDebug**: Compile-gated, engine-neutral terrain/climate/biome statistics and RGBA debug visualization; biome colours are supplied by the application with a neutral engine fallback
...
### World Simulation Climate And Weather
- **Seasonal temperature evaluation**: `world::SeasonalTemperatureMap` is a versioned derived map that evaluates predictable temperature offsets from annual temperature normals, stable terrain coordinates, explicit seasonal settings, profile identity, and normalized year fraction. It is not part of procgen climate identity and does not affect biome assignment.
- **Seasonal precipitation evaluation**: Future world-simulation/application climate work will evaluate wet/dry seasonal tendencies from annual precipitation normals without treating climatological transport as runtime wind.
- **Runtime atmosphere**: Future mutable weather state owns current temperature anomaly, pressure, runtime wind vector, humidity, cloud cover, active precipitation, storm/drought metadata, and simulation timestamp. It advances on a fixed simulation cadence or explicit event schedule rather than render frames.
- **Persistence**: Annual climate and biome maps remain regenerated from versioned inputs by default. Seasonal samples can be recomputed from calendar/profile inputs. Runtime weather, runoff, soil moisture, and discharge are persisted only as world/region simulation state once the region lifecycle and persistence systems own them.
- **Queries**: Gameplay-facing queries should distinguish stable annual climate, seasonal tendency, runtime atmospheric state, composed experienced conditions, and active precipitation so consumers do not accidentally treat weather as biome-defining climate.
